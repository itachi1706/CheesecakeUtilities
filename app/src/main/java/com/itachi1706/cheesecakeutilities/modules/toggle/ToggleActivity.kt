...
import com.itachi1706.cheesecakeutilities.databinding.ActivityToggleBinding
...
class ToggleActivity : BaseModuleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.togglePrivateDns.visibility = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) View.GONE else View.VISIBLE
        ...
        binding.toggleSpinnerPrivateDns.onItemSelectedListener = object: AdapterView.OnItemSelectedListener {
        ...
        binding.toggleSpinnerPrivateDns.setSelection(sp.getInt(PRIVATE_DNS_SETTING, 0))
        binding.toggleSwitchPrivateDns.setOnCheckedChangeListener { buttonView, isChecked ->
            ...
            val onState = resources.getStringArray(R.array.private_dns_entries_option)[binding.toggleSpinnerPrivateDns.selectedItemPosition]
        ...
        binding.toggleStatusbarPrivateDns.isChecked = checkPrivDNSTileEnabled()
        binding.toggleStatusbarPrivateDns.setOnCheckedChangeListener { _, isChecked -> packageManager.setComponentEnabledSetting(ComponentName(this, QSPrivateDNSTileService::class.java),
    ...
    private fun hasStuffOnScreen(): Boolean { return binding.togglePrivateDns.visibility != View.GONE }
    ...
    private fun checkPrivateDns() {
        ...
        setTextColor(binding.toggleStatusPrivateDns, isAllowed)
        binding.toggleStatusPrivateDns.text = if (isAllowed) "Granted" else "Not Granted"
        binding.toggleSpinnerPrivateDns.isEnabled = isAllowed
        binding.toggleStatusbarPrivateDns.isEnabled = isAllowed
        binding.toggleSwitchPrivateDns.isEnabled = isAllowed
        ...
        // Get selected option
        val pos = binding.toggleSpinnerPrivateDns.selectedItemPosition
        val selection = resources.getStringArray(R.array.private_dns_entries_option)[pos]

        privateDnsUpdater = true
        binding.toggleSwitchPrivateDns.isChecked = option == selection // check if match
    }
    ...
    private fun checkPixelForce90Hz() {
        ...
        binding.toggleGrantForce90hz.setOnClickListener {
        ...
        binding.toggleSwitchForce90hz.setOnCheckedChangeListener { buttonView, isChecked ->
        ...
        binding.toggleStatusbarForce90hz.isChecked = checkForce90HzTileEnabled()
        binding.toggleStatusbarForce90hz.setOnCheckedChangeListener { _, isChecked -> packageManager.setComponentEnabledSetting(ComponentName(this, QSForce90HzTileService::class.java),
                if (isChecked) PackageManager.COMPONENT_ENABLED_STATE_ENABLED else PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP) }
    ...
    private fun refreshPixelForce90Hz() {
        if (binding.toggleForce90hz.visibility == View.GONE) return // Not doing anything
        // Check Settings
        val option = Settings.System.getFloat(contentResolver, FORCE_90HZ_SETTING)
        LogHelper.d(TAG, "Current Min Refresh Rate: $option")

        if (binding.toggleSwitchForce90hz.isChecked != (option == 90.0f)) force90HzUpdater = true
        binding.toggleSwitchForce90hz.isChecked = option == 90.0f // 90Hz display
    ...
    private fun isAllowedPixel(isAllowed: Boolean, version: String) {
        setTextColor(binding.toggleStatusForce90hz, isAllowed)
        binding.toggleStatusForce90hz.text = if (isAllowed) "Installed ($version)" else "Not Installed"
        binding.toggleSwitchForce90hz.isEnabled = isAllowed
        binding.toggleStatusbarForce90hz.isEnabled = isAllowed
        binding.toggleGrantForce90hz.text = if (isAllowed) "Update Companion App" else "Install Companion App"
    }
    ...
    private fun disablePixelMode(msg: String) {
        LogHelper.w(TAG, msg)
        binding.toggleForce90hz.visibility = View.GONE
    }
